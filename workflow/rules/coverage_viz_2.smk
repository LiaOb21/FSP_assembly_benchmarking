rule coverage_viz_2:
    ...
    output:
        coverage_plot=f"{output_dir}" + "{sample}/coverage_viz_pilon/{assembler}/{sample}_{assembler}_coverage_plot.png",
        coverage_summary=f"{output_dir}" + "{sample}/coverage_viz_pilon/{assembler}/{sample}_{assembler}_coverage_summary.txt",
    threads: 1
    log:
        "logs/{sample}/coverage_viz_{assembler}_pilon.log",
    benchmark:
        "benchmark/{sample}/coverage_viz_{assembler}_pilon.txt"
    conda:
        "../envs/data_viz.yaml"
    shell:
        """
