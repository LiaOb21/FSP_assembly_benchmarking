...
rule abyss:
    ...
    shell:
        ...
        echo "abyss-pe -C {output.result_dir} name=abyss k={params.k} B={params.B} in='{input.forward_in} {input.reverse_in}' se='{input.merged_in}' -j {threads} {params.optional_params}" >> {log} 2>&1
        abyss-pe -C {output.result_dir} name=abyss k={params.k} B={params.B} in='{input.forward_in} {input.reverse_in}' se='{input.merged_in}' -j {threads} {params.optional_params} >> {log} 2>&1
