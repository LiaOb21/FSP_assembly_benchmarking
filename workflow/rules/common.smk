...
#        expand(f"{output_dir}" + "{sample}/best_assembly/pilon/{sample}_best_assembly_pilon.fasta", sample=SAMPLES),
        expand(f"{output_dir}" + "{sample}/best_assembly_qc/busco_general_pilon", sample=SAMPLES),
        expand(f"{output_dir}" + "{sample}/best_assembly_qc/busco_specific_pilon", sample=SAMPLES),
        expand(f"{output_dir}" + "{sample}/best_assembly_qc/merquryfk_pilon/merquryfk.completeness.stats", sample=SAMPLES),
        expand(f"{output_dir}" + "{sample}/best_assembly_qc/merquryfk_pilon/merquryfk.qv", sample=SAMPLES),
        expand(f"{output_dir}" + "{sample}/best_assembly_qc/quast_pilon", sample=SAMPLES),
        expand(f"{output_dir}" + "{sample}/best_assembly_qc/bwa_mem2_samtools_pilon/{sample}_best_assembly_pilon_sorted.bam", sample=SAMPLES),
...
def validate_megahit_kstep(k_list, max_kstep):
    ...
    for i in range(1, len(k_list)):
        ...
        if current_k - prev_k <= max_kstep:
            ...
            print(f"Warning: Skipping k-mer {current_k} for MEGAHIT (step {current_k - prev_k} > {max_kstep})")
