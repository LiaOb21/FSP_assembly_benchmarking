...
rule minia:
    ...
    params:
        k = config["minia"]["k"],
        result_prefix = lambda wildcards, output: os.path.splitext(output.scaffolds)[0].replace('.contigs', ''),
        file_list = lambda wildcards, output: os.path.join(os.path.dirname(output.scaffolds), f"{wildcards.sample}_files.txt"),
